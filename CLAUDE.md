...
- [x] **Fase 9.2** — Comentário mais humano + separação por sessão. `templates.py` re-estruturado para `TEMPLATES[lang][session_type][event_key]` com buckets `practice`/`qualifying`/`race` por cada uma das 4 línguas. Race pool reescrita com tom conversacional (contractions, interjeições, frases incompletas). Practice pool nova (analítica, calma: "reference lap", "engineers will want a chat"). Qualifying pool nova (tensa, foco em volta única: "provisional pole", "out for the flyer", "banker lap", "all or nothing"). `_session_key()` normaliza strings "Qualify 2"/"Practice 1"/"Race" para o bucket. Fallback session→race→flag_generic. `ai_commentator.py`: `SESSION_TONE` reescrito com proibições duras (quali NUNCA diz "race"/"laps to go"; practice NUNCA diz "lights out"/"final stages"). `SPEAKER_PERSONA` expandido. Novo `STYLE_DIRECTIVE` injectado no system prompt obriga o LLM a soar como broadcaster ao vivo (não news reader). `generate()` encaminha `session_type` também para o template offline. 40/40 testes a passar.
